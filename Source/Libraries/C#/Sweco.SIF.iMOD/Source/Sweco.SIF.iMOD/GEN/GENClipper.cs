...
{
    /// <summary>
    /// Class for clipping GEN-lines and GEN-polygons
    /// </summary>
    /// <remarks>
    /// Code from: http://rosettacode.org/wiki/Sutherland-Hodgman_polygon_clipping 
    /// and https://en.wikipedia.org/wiki/Cohen%E2%80%93Sutherland_algorithm
    /// </remarks>
    public class GENClipper
    {
        /// <summary>
        ...
        public static List<Point> ClipPolygon(List<Point> subjectPolygon, List<Point> clipPolygon)
        {
            ...
            {
                if (ex.GetBaseException().Message.Contains("colinear"))
                {
                    // ignore
                }
                else
            ...
            foreach (LineSegment clipEdge in IterateEdgesClockwise(clipPolygon))
            {
                ...
                foreach (Point E in inputList)
                {
                    if (clipEdge.IsInside(E))
                    {
                        ...
                        {
                            Point point = Intersect(S, E, clipEdge.P1, clipEdge.P2);
                            if (point == null)
                            {
        ...
        //    double dotPerp = ((double)direction1.dX * (double)direction2.dY) - ((double)direction1.dY * (double)direction2.dX);
        ...
        //    double t = (double) (((double)c.dX * ((double)direction2.dY - (double)c.dY * (double)direction2.dX) / dotPerp));
        ...
        /// <summary>
        /// Cohen–Sutherland clipping algorithm clips a line from P1 = (x1, y1) to P2 = (x2, y2) against a rectangle with 
        /// diagonal from clipExtent.
        /// </summary>
